...
const JobViewPage = () => {
  ...
  return (
    <div data-theme={profile?.data.theme || "light"} className="flex">
      {openApplyforJob && job ? (
        <ApplyForJob job={job} setOpenApplyforJob={setOpenApplyforJob} />
      ) : null}
      <NavigationBar />
      <div className="job-details-container px-10 w-full">
        <div className="header mt-10 flex">
          ...
          <div className="job-title flex flex-col w-full  relative">
            <div className="flex  items-center w-full">
              ...
              {job?.companyId === profile?.data.userId ||
              job?.recruiterId === profile?.data.userId ? (
                <EditDropdown getJob={getJob} job={job} />
              ) : null}
            </div>
            <span>
              <span
        ...
        <div className="job-details mt-10">
          <div
            className="apply-btn btn btn-primary text-white cursor-pointer"
            onClick={() => setOpenApplyforJob(true)}
          >
            Apply
          </div>
