...
import { CssBaseline, MenuItem, Select, SelectChangeEvent, ThemeProvider } from "@mui/material";
...
const PostChart = () => {
  ...
  const fetchData = async (range: string, customRange?: { start: string, end: string }) => {
    try {
      let url = `${URL}/post/get/chart/post/data?range=${range}`;
  ...
  const handleCustomRangeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setCustomRange({
      ...customRange,
    ...
    <>
      <div className="w-[90%] relative mr-7 p-2  border-dashed rounded-md border-[.5px] border-slate-50 h-[200px] sm:h-[500px]">
        <div className="controll-unit w-full flex justify-between">
          <div className="name"><span className="font-semibold text-3xl">Posts</span></div>
          <div className="dropdown w-52">
          <ThemeProvider theme={darkTheme}>
          <CssBaseline />
          <Select  fullWidth value={timeRange} onChange={handleTimeRangeChange}>
          ...
          {openSelectTimeRange && (
        <SelectTimeRange customRange={customRange} handleCustomRangeChange={handleCustomRangeChange} setOpenSelectTimeRange={setOpenSelectTimeRange}/>
      )}
          </div>
        </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          <Tooltip content={<CustomToolTip title="Posts"/>}/>
          {/* <Legend/> */}
            <Area type="monotone" stroke="#25632b" fill="#3b82f6" dataKey="userCount" />
          </AreaChart>
        </ResponsiveContainer>
        </div>
      </div>
