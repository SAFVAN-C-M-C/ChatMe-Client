...
const CompanyJobs:FC<CompanyJobsProps> = ({userId}) => {
  ...
  const getUserjob=async()=>{
    try {
      ...
      const res=await axios.get(`${URL}/job/get/user/${String(userId)}`,config)
      if(res.status===200){
  ...
  return (
    ...
    {
      userJob && userJob.length>0?userJob.map((job,index)=>(<JobPost job={job} key={index}/>)):(<span>No Jobs here</span>)
    }
    </div>
  )
}
