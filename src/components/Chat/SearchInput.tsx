...
const SearchInput: FC<SearchInputProps> = ({ serachRef }) => {
  ...
  const getUsers = async (key: string) => {
    const { data } = await axios.get(
      `${URL}/profile/users/search?key=${key}`,
      config
    );
  ...
  return (
    <>
      ...
      {searchData && searchData.length !== 0 ? (
        <div
          className={`w-auto left-[80px] px-14 pt-3 pb-3 h-auto  absolute z-50 ${
            profile?.data.theme === "dark" ? "bg-gray-600" : "bg-slate-300"
          } overflow-y-auto flex flex-col`}
        >
          {searchData?.map((user, index) => (
            <SearchUser user={user} key={index} setSearchData={setSearchData} />
          ))}
        </div>
      ) : null}
    </>
