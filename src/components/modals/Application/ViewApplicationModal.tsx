...
import { Box, Button, CssBaseline, Dialog, DialogActions, DialogContent, DialogTitle, MenuItem, Select, SelectChangeEvent, Typography } from '@mui/material';
...
const ViewApplicationModal:FC<ViewApplicationModalProp> = ({setOpenViewApplicationModal,application,user,fetchApplications}) => {
    ...
    useEffect(()=>{
        const updateStatus=async(applicationId:string)=>{
            try {
                ...
                const response = await axios.put(`${URL}/job/applications/update/${applicationId}`,formData, config);
      ...
      >
        ...
        <DialogContent>
          <div className='w-full h-[300px] lg:h-[500px]'>
            ...
            <div className="email">
                Email: <span className='font-semibold'>{application.email}</span>
            </div>
            <div className="phone">
                Phone: <span className='font-semibold'>{application.phone}</span>
            </div>
            <div className="applied-at">
                Applied on: <span className='font-semibold'>{formatDate(application.createdAt)}</span>
            </div>
